...
const NavLink = ({title}) => (
    <LinkScroll className='base-bold text-p4 uppercase cursor-pointer transition-colors duration-1000 hover:text-p1 max-lg:my-4 max-lg:h5'>    
        {title}
    </LinkScroll>
)
...
const Header = () =>  {
    ...
    useEffect(() => {
        const handleScroll = () => {
            setHasScroll(window.scrollY > 35)
        }
        ...
        <header className={clsx( 'fixed top-0 left-0 z-50 w-full py-10 transition-all duration-500 max-lg:py-4', hasScroll && 'py-2 bg-black-100 backdrop-blur-[8px]' )}  >
            <div className='container flex h-14 items-center max-lg:px-5'>
                ...
                <div className= {clsx('w-full max-lg:fixed max-lg:top-0 max-lg:left-0 max-lg:w-full max-lg:bg-s2 max-lg:opacity-0', isOpen ? 'max-lg:opacity-100' : 'max-lg:pointer-events-none')}>
                    <div className="mx-lg:relative max-lg:flex max-lg:flex-col max-lg:min-h-screen max-lg:p-6 max-lg:overflow-hidden sidebar-before max-md:px-4">
                        <nav className='max-lg:relative max-lg:z-2 max-lg:my-auto'>
                            <ul className='flex max-lg:block max-lg:px-12'>
                                ...
                                <li className='nav-logo'>
                                    <LinkScroll to='hero' offset={-100} smooth spy className={clsx('max-lg:hidden transition-transform duration-500 cursor-pointer')}>
                        ...
                        <div className="lg:hidden block absolute top-1/2 left-0 w-[960px] h-[380px] translate-x-[-290px] -translate-y-1/2 rotate-90">
                            {/* <img src="/images/bg-autlines.svg" width={960} height={380} alt="outline" className='relatice z-2 '/> */}
