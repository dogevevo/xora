...
import Butom from '../components/Butom'
const Features = () => {
  return (
    <section> 
        <Element name='features'>
            <div className="container">
                <div className="relative flex md:flex-wrap flex-nowrap border-2 border-s3 rounded-3xl md:overflow-hidden max-md:flex-col feature-after md-g7 max-md:border-none max-md:rounded-none max-md:gap-3">
                    {features.map(({id, icon, caption, title, text, button}) => (
                        <div className="relative z-2 md:px-10 px-5 md:pb-10 pb-5 flex-50 max-md:g7 max-md-border-2 max-md:border-s3 max-md:rounded-3xl max-md:flex-320 " key={id}>
                            ...
                            <Butom icon={button.icon}>{button.title}</Butom>
                        </div>
                    ))}
                    <ul className='relative flex justify-around flex-grow px-[5%]  border-s3 rounded-7xl max-md:hidden'>
